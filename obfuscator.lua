...
local protected = xor(dumped, key):reverse()
...
local encoded = enc(protected)

-- split into random parts and shuffle
local parts = {}
local i = 1
while i <= #encoded do
  local len = math.random(15,30)
  parts[#parts+1] = encoded:sub(i, i+len-1)
  i = i + len
end
...
local pstr = {}
for i=1,#parts do pstr[i]=string.format('%q', parts[i]) end
local partsStr = table.concat(pstr, ',')
local tmpl = [=[local key=%q
local b=%q
local parts={%s}
...
local chunk=assert(load(xor(string.reverse(dec(encoded)),key)))
chunk(...)
]=]
out:write(string.format(tmpl, key, b, partsStr))
out:close()
